...
public class MaxBinaryHeap<T extends Comparable<T>> {
    ...
    public MaxBinaryHeap(final Class<T> componentType) {
        this(componentType, 9);
    }

    public MaxBinaryHeap(final Class<T> componentType, int capacity) {
    ...
    private void shiftDown(int index, T val) {
        ...
        if (right == null) {
            if (this.isLessThan(val, left)) {
                this.values[leftIndex] = val;
                this.values[index] = left;
            }
        } else if (isLessThan(left, right)) {
            if (this.isLessThan(val, right)) {
    ...
    public static void main(String[] args) {
        ...
        int max = heap.get();
        System.out.printf("Getting %d%n", max);
        int num = 2;
        for (int i = 0; i < num; i++) {
            int val = heap.get();
            if (val > max) {
                throw new RuntimeException("Max was less than val");
            }
